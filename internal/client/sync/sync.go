...
type DiffItem struct {
	Key       string            `json:"key"`
	LocalVal  *resource.ValJson `json:"local_val,omitempty"`
	RemoteVal *resource.ValJson `json:"remote_val,omitempty"`
...
func ComputeDiff(local, remote map[resource.ValJsonKey]resource.ValJson) DiffResult {
	result := DiffResult{
		LocalOnly:  []DiffItem{},
		RemoteOnly: []DiffItem{},
		Conflicts:  []DiffItem{},
	...
	remoteByKey := make(map[string]resource.ValJson)
...
func valJsonEqual(a, b resource.ValJson) bool {
	if a.Val != b.Val {
		return false
	}
...
func ExecutePull(diff DiffResult, localStorage, remoteStorage corestorage.Storage, dryRun bool) error {
	...
	for _, item := range diff.LocalOnly {
		...
		vjk := resource.ValJsonKey{Key: item.Key, Type: resource.ORIGIN}

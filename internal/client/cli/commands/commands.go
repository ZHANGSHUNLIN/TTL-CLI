...
var AddCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		value := text.UnescapeString(args[1])

		newResource := resource.ValJson{
			Val: value,
			Tag: text.RemoveDuplicates(addTags),
		}
...
var GetCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return PossiblyRun(debug, "get", resources, args[0], func(key resource.ValJsonKey, json resource.ValJson) error {
			Println()
...
var RenameCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		newKey := resource.ValJsonKey{
			Key:  args[1],
			Type: resource.ORIGIN,
		}

		if err := clientService(cmd).SaveResource(newKey, value); err != nil {
...
var ConfigCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if confFile == "" {
			confFile, err = config.GetDefaultConfPath()
		}
		if err == nil {
...
var UpdateCmd = &cobra.Command{
	...
	Args:    cobra.ExactArgs(2),
	RunE: func(cmd *cobra.Command, args []string) error {
		key := resource.ValJsonKey{Key: args[0], Type: resource.ORIGIN}
		debug := cmd.Context().Value("debug").(bool)
		...
		value := text.UnescapeString(args[1])
		return clientService(cmd).UpdateResource(key, resource.ValJson{Val: value, Tag: existing.Tag})
	},
}

func PossiblyRun(debug bool, opt string, resources map[resource.ValJsonKey]resource.ValJson, searchKey string,
	action func(resource.ValJsonKey, resource.ValJson) error) error {

	if debug {
		Println("当前是", opt, "操作")
	}

	matches := make(map[resource.ValJsonKey]resource.ValJson)
	for key, val := range resources {
		if text.ContainsIgnoreCase(key.Key, searchKey) {
			matches[key] = val
		}

		for _, s := range val.Tag {
			if text.ContainsIgnoreCase(s, searchKey) {
				matches[key] = val
			}
		}
	}
	...
	keys := make([]resource.ValJsonKey, 0, len(matches))
	for key := range matches {
	...
	order := make([]resource.ValJsonKey, 0, len(matches))
	idx := 1

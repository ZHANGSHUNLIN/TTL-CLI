...
var WorkspaceListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		names, current, err := config.ListWorkspaces(confFile)
		if err != nil {
		...
		for _, name := range names {
			...
			dbPath, storageType, count, err := config.GetWorkspaceInfo(confFile, name)
			if err == nil && dbPath != "" {
				if count > 0 {
...
var WorkspaceSwitchCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		current, err := config.GetCurrentWorkspace(confFile)
		if err == nil && current == name {
			Println(i18n.T("command.workspace.switch.already_current", name))
			return nil
		}

		if err := config.SwitchWorkspace(confFile, name); err != nil {
			return err
		}
...
var WorkspaceCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		dbPath, err := config.CreateWorkspace(confFile, name)
		if err != nil {
			return err
		}
...
var WorkspaceDeleteCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := config.DeleteWorkspace(confFile, name); err != nil {
			return err
		}
...
var WorkspaceCurrentCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		confFile := cmd.Context().Value("confFile").(string)
		workspaceName, err := config.GetCurrentWorkspace(confFile)
		if err != nil {
			return err
		}

		dbPath, storageType, count, err := config.GetWorkspaceInfo(confFile, workspaceName)
		if err != nil {
			return err
		}

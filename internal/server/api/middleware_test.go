...
func setupMultiTenantTest(t *testing.T) (*tenant.UserStore, *tenant.StorageManager, func()) {
	t.Helper()
	tmpDir := t.TempDir()

	store := tenant.NewUserStore(filepath.Join(tmpDir, "users.json"))
	if err := store.Load(); err != nil {
		t.Fatal(err)
	}

	tenantMgr := tenant.NewStorageManager(filepath.Join(tmpDir, "tenants"))
...
func TestMultiTenantAuth_ValidKey(t *testing.T) {
	...
	var capturedStorage corestorage.Storage
